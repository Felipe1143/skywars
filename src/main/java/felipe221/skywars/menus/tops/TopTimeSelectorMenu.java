...
public class TopTimeSelectorMenu {
    public static void open(Player player) {
        int ROWS = MenuLoad.Menus.TOPS_TIME_SELECTOR.getRows();
        String TITLE = MenuLoad.Menus.TOPS_TIME_SELECTOR.getTitle();
        Inventory menu = Bukkit.createInventory(player, 9 * ROWS, TITLE);

        for (Map.Entry<Integer, ItemStack> entry : MenuLoad.Menus.TOPS_TIME_SELECTOR.getItemsWithSlot().entrySet()) {
